class Customers::FarmersController < ApplicationController
  ...
  def show
    ...
    @recipes = Recipe.where(farmer_id: params[:id]).order('created_at DESC').first(4)
    @events = Event.where(farmer_id: params[:id]).where('end_date > ?', Date.current).order('created_at DESC').first(4)

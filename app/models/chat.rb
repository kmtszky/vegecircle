class Chat < ApplicationRecord
  ...
  def notice_created_by(farmer)
    chat_participants_ids = Chat.where(event_id: event_id).pluck(:customer_id)
    schedules_ids = Schedule.select(:id).where(event_id: event_id)
    ...
    if recipients.present?
      notice = Notice.new(farmer_id: farmer.id, event_id: event_id, action: "チャット")

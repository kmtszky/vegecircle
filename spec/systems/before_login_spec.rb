...
describe '[STEP1] ユーザログイン前のテスト' do
  ...
  describe 'ヘッダーのテスト: ログインしていない場合' do
    ...
    context '表示内容の確認' do
      it 'Homeリンクが表示される: 左上から1番目のリンクが"ロゴマーク"である' do
        home_link = find_all('a')[1].native.inner_text
        expect(home_link).to match('')
      end
      it 'Aboutリンクが表示される: 左上から2番目のリンクが「vegecircleについて」である' do
      ...
      it 'Customer用Log inリンクが表示される: 左から6番目のリンクが「ログイン」である' do
        customer_log_in_link = find_all('a')[6].native.inner_text
        expect(customer_log_in_link).to match(/sign_in/i)
      end
      it 'Customer用Log inリンクの内容が正しい' do
        customer_log_in_link = find_all('a')[6].native.inner_text
        expect(page).to have_link customer_log_in_link, href: new_customer_session_path
      end
    end
